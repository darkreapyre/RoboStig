...
# Based on https://github.com/aws/sagemaker-chainer-container/blob/master/src/sagemaker_chainer_container/training.py
...
def train(env, hyperparameters, resources):
    """
    Runs Horovod training on a user supplied module in distributed SageMaker environment.
    ...
    * `sagemaker_additional_mpi_options`: a string of options to pass to mpirun. [NOT USED]
    ...
    """
    # `sagemaker_use_mpi` by default for distributed training
...
def _create_mpi_script(env, hyperparameters):
    ...
    Args:
        env: an instance of the training environment.
        hyperparameters: subset of env
    """
    # Return python list of command arguments
...
def _get_mpi_command(env, hyperparameters, resources):
    ...
    * -mca btl_tcp_if_include [env.network_interface_name]: Tell OpenMPI to use
        the given network interface name for byte transfer layer communication. <-- Not working
    * -mca oob_tcp_if_include [env.network_interface_name]: Tell OpenMPI to use
        the given network interface name for out-of-band communication.  <-- Not working
    ...
    * -mca orte_abort_on_non_zero_status 1: Return a non-zero exit code if any process exits
        with a non-zero exit code. <-- Not working
...
def _run_mpi_on_all_nodes(env, hyperparameters, resources):
    ...
    #framework.logging.log_script_invocation(cmd, env.to_env_vars(), logger) #<-- Need to Investigate sageMaker-containers
...
def _wait_for_training_to_finish(env):
    ...
    print("MPI training process on worker node %s -- Started" % current_host)

    _wait_until_mpi_stops_running()
    
    print("Training process on worker node %s -- Stopped" % current_host)
